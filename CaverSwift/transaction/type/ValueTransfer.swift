...
open class ValueTransfer: AbstractTransaction {
    ...
    public class Builder: AbstractTransaction.Builder {
        ...
        public init() {
            super.init(TransactionType.TxTypeValueTransfer.string)
        }
        
        public override func build() throws -> ValueTransfer {
            return try ValueTransfer(self)
        }
    ...
    public init(_ klaytnCall: Klay?, _ from: String, _ nonce: String = "0x", _ gas: String, _ gasPrice: String = "0x", _ chainId: String = "0x", _ signatures: [SignatureData]?, _ to: String, _ value: String) throws {
        try super.init(klaytnCall, TransactionType.TxTypeValueTransfer.string, from, nonce, gas, gasPrice, chainId, signatures)
        try setTo(to)
        try setValue(value)
    }
    
    public static func decode(_ rlpEncoded: String) throws -> ValueTransfer {
    ...
    public static func decode(_ rlpEncoded: [UInt8]) throws -> ValueTransfer {
        if rlpEncoded[0] != TransactionType.TxTypeValueTransfer.rawValue {
        ...
        guard let values = rlpList as? [Any],
              ...
              let senderSignatures = values[6] as? [[String]] else {
    ...
    public override func compareTxField(_ txObj: AbstractTransaction, _ checkSig: Bool) -> Bool {
        ...
        guard let txObj = txObj as? ValueTransfer else { return false }
