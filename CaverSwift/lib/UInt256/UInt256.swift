...
//      self.value = [0,0,0,0,0,0,leftDigit, rightDigit]
    ...
    public init(hexStringValue: String) {
        ...
        assert(hexStringValue.count > 0, "Can't be empty");
        
        // Assert if string isn't too long
        assert(hexStringValue.count <= 64, "Too large");
        ...
        // Pad zeros
        if hexStringValue.count < 64 {
            for _ in 1...(64 - hexStringValue.count) {
    ...
    public init(decimalStringValue: String) {
        ...
        assert(decimalStringValue.count > 0, "Can't be empty");
        
        // Assert if string longer than 78 characters
        assert(decimalStringValue.count <= 78, "Too large");
        ...
        if decimalStringValue.count < 78 {
            for _ in 1...(78 - decimalStringValue.count) {
    ...
    public var toData: Data {
        ...
        return Data(bytes: val, count: 32)
    }
}
