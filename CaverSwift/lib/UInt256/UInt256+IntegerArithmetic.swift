...
extension UInt256 {
    ...
    public static func divideWithOverflow(_ numerator: UInt256, _ denominator: UInt256) -> (UInt256, overflow: Bool) {
        assert(denominator != UInt256(0), "Divide by zero")
        ...
        guard let result = divideWithOverflowC(&num, &den) else {return (UInt256(0), false)}
...
//        return (remainder, false)
    ...
    public func modInverse(_ m: UInt256) -> UInt256 {
        assert(m > UInt256(0), "Modulo 0 makes no sense")
...
public func * (lhs: UInt256, rhs: UInt256) -> UInt256 {
    ...
    if (lhs == zero) || (rhs == zero) {
        return UInt256(0) // Don't use lhs == 0, because it casts to a signed 32 bit integer
    }
...
public func * (lhs: UInt256, rhs: UInt256) -> (UInt256, UInt256) {
    ...
    if lhs == UInt256(0) || rhs == UInt256(0) {
    ...
    if x₁_plus_x₀ & UInt256.singleBitAt(127) != UInt256(0) || y₁_plus_y₀ & UInt256.singleBitAt(127) != UInt256(0)  {
        ...
        if left == UInt256(0) { // right represents the full value of z₁subtotal, so this will not overflow:
            z₁ = right - z₂ - z₀
        } else {
            ...
            if addSafe >= z₂ { // z₂ + z₀ doesn't overflow
                if right >= addSafe { // subtraction won't overflow
                    ...
                    z₁tuple = (left - UInt256(1), right &- addSafe)
                }
            } else { // z₂ + z₀ overflows
                if right >= addSafe { // subtraction won't overflow again
                    ...
                    z₁tuple = (left - UInt256(2), right &- addSafe)
                }
            }
        }
