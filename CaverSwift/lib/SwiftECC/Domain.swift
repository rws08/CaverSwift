...
public class Domain: CustomStringConvertible {
    ...
    public func encodePoint(_ p: Point, _ compress: Bool = false) throws -> [UInt8] {
        if !self.contains(p) {
    ...
    public func decodePoint(_ bytes: [UInt8]) throws -> Point {
    ...
    static func domainFromASN1(_ asn1: ASN1) throws -> Domain {
        ...
        let gx = BInt(magnitude: [UInt8](g.value[1 ..< l + 1]))
        let gy = BInt(magnitude: [UInt8](g.value[l + 1 ..< g.value.count]))
