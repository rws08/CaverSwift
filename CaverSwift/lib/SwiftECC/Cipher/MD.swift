...
protocol MessageDigestImpl {
    func doBuffer(_ buffer: inout [UInt8], _ hw: inout Words, _ hl: inout Longs)
    func doReset(_ hw: inout Words, _ hl: inout Longs)
    func padding(_ totalBytes: Int, _ blockSize: Int) -> [UInt8]
}
...
class MessageDigest {
    ...
    init(_ algorithm: MessageDigestAlgorithm) {
        ...
        case .SHA2_224:
            ...
            self.buffer = [UInt8](repeating: 0, count: 64)
        ...
        case .SHA2_384:
            ...
            self.buffer = [UInt8](repeating: 0, count: 128)
    ...
    func digest() -> [UInt8] {
        var md = [UInt8](repeating: 0, count: self.digestLength)
        ...
        if self.digestLength > 32 {
            ...
            for i in 0 ..< self.digestLength {
                md[i] = UInt8((self.hl[i >> 3] >> ((7 - (i & 0x7)) * 8)) & 0xff)
            }
        } else {
            ...
            for i in 0 ..< self.digestLength {
                md[i] = UInt8((self.hw[i >> 2] >> ((3 - (i & 0x3)) * 8)) & 0xff)
            }
        }
        self.reset()
        return md
