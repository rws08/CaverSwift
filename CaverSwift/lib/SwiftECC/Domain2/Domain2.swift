...
class Domain2 {
    ...
    // [SEC 1] section 2.3.3
    func encodePoint(_ pt: Point, _ compress: Bool) -> [UInt8] {
        var b: [UInt8]
        if pt.infinity {
            b = [0]
        } else {
            ...
            if compress {
                ...
                b = [UInt8](repeating: 0, count: 2 * l + 1)
                b[0] = 4
                var x = 1 + l - bx.count
                for i in 0 ..< bx.count {
                    b[x] = bx[i]
    ...
    // [SEC 1] section 2.3.4
    func decodePoint(_ bytes: [UInt8]) throws -> Point {
        var p: Point
        let l = bytes.count
        ...
        if l == 1 {
            ...
            let x = BInt(magnitude: [UInt8](bytes[1 ..< l]))
            if x.isZero {
                var B = self.b
            ...
            let x = BInt(magnitude: [UInt8](bytes[1 ... l / 2]))
            let y = BInt(magnitude: [UInt8](bytes[l / 2 + 1 ..< l]))
            p = Point(x, y)
        } else {
            throw ECException.decodePoint
        }
