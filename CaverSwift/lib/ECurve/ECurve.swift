...
public struct ECurve {
    ...
    public var description: String {
        return "Curve over \(field) with base point (\(gX.value), \(gY.value)), a = \(a), b = \(b), order \(n) and cofactor \(String(describing: h))"
    }
...
public func * (lhs: UInt256, rhs: ECPoint) -> ECPoint {
    ...
    if lookup != nil {
        ...
        for incrementId in lookup! {
            ...
            if UInt256.singleBitAt(255 - i) & lhs != UInt256(0) {
...
public func importLookupTable () -> [Any] { // ECPoint array crashes compiler with -Ounchecked
    ...
    */
    ...
    for coordinates in plist {
        guard let coordinates = coordinates as? [Any] else { return lookup }
        let x  = coordinates[0] as! [NSNumber]
        let y  = coordinates[1] as! [NSNumber]
        
        let X = field.int(UInt256(x[0].uint32Value, x[1].uint32Value, x[2].uint32Value, x[3].uint32Value, x[4].uint32Value, x[5].uint32Value, x[6].uint32Value, x[7].uint32Value))
        
        let Y = field.int(UInt256(y[0].uint32Value, y[1].uint32Value, y[2].uint32Value, y[3].uint32Value, y[4].uint32Value, y[5].uint32Value, y[6].uint32Value, y[7].uint32Value))
